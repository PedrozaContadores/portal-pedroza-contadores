(() => {
  ...
  fetch('../../data/conteudo-fiscal.json').then(r=>{if(!r.ok) throw new Error(); return r.json();}).then(data=>{items=data.itens; const categories=['Todos',...new Set(items.map(i=>i.categoria))]; filters.innerHTML=categories.map(c=>`<button class="fiscal-filter${c==='Todos'?' is-active':''}" type="button" data-category="${escapeHtml(c)}">${escapeHtml(c)}</button>`).join(''); filters.addEventListener('click',e=>{const b=e.target.closest('button');if(b)setCategory(b.dataset.category)}); const params=new URLSearchParams(location.search); const requested=params.get('categoria'); const query=params.get('q'); if(query&&search){search.value=query.replace(/-/g,' '); render();} else if(requested&&categories.includes(requested))setCategory(requested); else render();}).catch(()=>{empty.hidden=false;empty.querySelector('strong').textContent='Não foi possível carregar a biblioteca.';});
